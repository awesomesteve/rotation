...
function initMedalSparkles() {
  ...
  document.querySelectorAll('.badge-row .medal-mini').forEach((svg, i) => {
    ...
    // Position it relative to the medal svg
      ...
      <div class="meta">
        <div class="name-row">
          ...
          ${(() => {
            ...
            const cycleLabel = {period:'🩸 period', peak:'⚠️ peak', ovulation:'⚠️ ovulation', fertile:'fertile', elevated:'elevated'}[todayCycle] || '';
            const cycleBadge = (todayCycle && todayCycle !== 'safer' && cycleLabel)
              ? `<span class="cycle-stage-badge ${todayCycle}" title="${{period:'On her period',peak:'Peak fertility — high pregnancy risk',ovulation:'Ovulation day — peak risk',fertile:'Fertile window (high risk)',elevated:'Elevated risk day'}[todayCycle]}">${cycleLabel}</span>` : '';
...
if ('contacts' in navigator && 'ContactsManager' in window) {
  ...
  btn.addEventListener('click', async () => {
    try {
      ...
      showView('editor');
    } catch(e) { alert('Contact picker not available.'); }
  });
}
...
function buildAvailRow(p, dayKey, jsDate) {
  ...
  if (cycleClass && cycleClass !== 'safer') {
    const flag = document.createElement('div');
    flag.className = 'cycle-flag ' + cycleClass;
    const cycleLabels = {period:'🩸 period', peak:'⚠️ peak', ovulation:'⚠️ ovulation', fertile:'fertile', elevated:'elevated'};
    const cycleTitles = {period:'On her period', peak:'Peak fertility — high pregnancy risk', ovulation:'Ovulation day — peak risk', fertile:'Fertile window (high risk)', elevated:'Elevated risk day'};
...
(function wireFortunesAndOpeners() {
  function fortune() { if (TOASTS.length) showToast(TOASTS[Math.floor(Math.random() * TOASTS.length)]); }
  function opener()  { if (TINDER_OPENERS.length) showToast(TINDER_OPENERS[Math.floor(Math.random() * TINDER_OPENERS.length)]); }
