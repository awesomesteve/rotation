...
// ── 5. Long-press 🛠 tools tab 1.5s → nuke dates banner ──────────
(function eggNukeDates() {
  const toolsBtn = document.querySelector('nav.tabs button[data-view="tools"]');
  ...
  function cancelHold() { clearTimeout(holdTimer); }
  toolsBtn.addEventListener('mousedown',  startHold);
  toolsBtn.addEventListener('touchstart', startHold, { passive: true });
  toolsBtn.addEventListener('mouseup',    cancelHold);
  toolsBtn.addEventListener('touchend',   cancelHold);
  toolsBtn.addEventListener('mouseleave', cancelHold);
  toolsBtn.addEventListener('click',      cancelHold);
