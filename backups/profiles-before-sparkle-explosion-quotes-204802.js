...
function _launchDaySparkle() {
  ...
  setTimeout(() => {
    ...
    const diamonds = diamondIcons().map(d => d); // the ::after is CSS — target the element itself for a filter flash
    const all = [...medals, ...diamonds];
    if (!all.length) return;
    const step = Math.min(1800, 19000 / all.length); // spread over 19s
    medals.forEach((el, i)  => burstElement(el, i * step));
    // For cream-icon diamonds, flash via the ::after by adding a transient class
